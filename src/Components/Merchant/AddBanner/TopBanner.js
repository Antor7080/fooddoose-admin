...
const TopBanner = () => {
  ...
  useEffect(() => {
    setLoading(true);
    authAxios.get(`/banner/merchantBanner/${user._id}?page=${page}`)
      .then((response) => {

        setAllBanner(response.data?.allBanner);
        setLoading(false);
      })
      .catch((error) => { });
  ...
  const handleDeActive = async (id) => {
    ...
    if (proceed) {
      authAxios
        .put(`/banner/status-update/${id}`, { status: "Deactive" })
        .then((data) => {
          /*  if (data?.data.success === true) {
             const remainFood = allBanner.filter((food) => food.id !== id);
             setAllBanner(remainFood);
                    ...
                    <div className="card h-100">
                      ...
                      <div className="card-foote p-3">
                        ...
                        <div className="d-flex align-items-center justify-content-around" style={{ minWidth: "100px" }}>
                          ...
                          {
                            (banner?.status === "Active" || banner?.status === "true") ? (<button title="Deactive" className="btn btn-warning" onClick={() => { handleDeActive(banner._id) }}>
                              <i
                      ...
                      allBanner.map((data) => (
                        <tr key={data._id}>
                          ...
                          <td className="d-flex align-items-center justify-content-around" style={{ minWidth: "100px" }}>
                            ...
                            {
                              (data?.status !== "Deactive") ? (<button title="Deactive" className="btn btn-warning" onClick={() => { handleDeActive(data._id) }}>
                                <i
                                  className="bi bi-toggle-on"
                                  style={{ color: "#660000" }}
                                ></i> Deactivate
